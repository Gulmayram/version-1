...
const Sidebar = () => {
  ...
  // Компонент для иконок, чтобы не дублировать код SVG
  const Icon = ({ name }) => {
    const icons = {
      main: (
      ...
      <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-menu">
          <ul>
            ...
            {/* О ПРЕДПРИЯТИИ */}
            <li className={`menu-item has-submenu ${activeSubmenu === 'about' ? 'submenu-open' : ''}`}>
              <div className="menu-link" onClick={() => toggleSubmenu('about')}>
                <div className="link-content">
                  <Icon name="about" /> <span>{getT('aboutCompany')}</span>
                </div>
                <span className="arrow">{activeSubmenu === 'about' ? '▲' : '▼'}</span>
              </div>
            ...
            {/* ДЕЯТЕЛЬНОСТЬ */}
            <li className={`menu-item has-submenu ${activeSubmenu === 'work' ? 'submenu-open' : ''}`}>
              <div className="menu-link" onClick={() => toggleSubmenu('work')}>
                <div className="link-content">
                  <Icon name="activity" /> <span>{getT('services')}</span>
                </div>
                <span className="arrow">{activeSubmenu === 'work' ? '▲' : '▼'}</span>
              </div>
              <ul className="submenu">
                <li>Услуги</li>
                <li>Проекты</li>
            ...
            {/* НОРМАТИВНАЯ БАЗА */}
            <li className={`menu-item has-submenu ${activeSubmenu === 'base' ? 'submenu-open' : ''}`}>
              <div className="menu-link" onClick={() => toggleSubmenu('base')}>
                <div className="link-content">
                   <Icon name="docs" /> <span>Нормативная база</span>
                </div>
                <span className="arrow">{activeSubmenu === 'base' ? '▲' : '▼'}</span>
              </div>
              <ul className="submenu">
                <li>Законодательство и НПА</li>
                <li>Формы и бланки</li>
                <li>Приложения и инструкции</li>
              </ul>
            </li>

            {/* ОБЪЯВЛЕНИЯ И НОВОСТИ */}
            <li className={`menu-item has-submenu ${activeSubmenu === 'news' ? 'submenu-open' : ''}`}>
              <div className="menu-link" onClick={() => toggleSubmenu('news')}>
                <div className="link-content">
                  <Icon name="news" /> <span>{getT('news')}</span>
                </div>
                <span className="arrow">{activeSubmenu === 'news' ? '▲' : '▼'}</span>
              </div>
              <ul className="submenu">
                <li>Конкурсы</li>
                <li>Закупки</li>
            ...
            <li className="menu-item">
              <div className="menu-link">
                <Icon name="contacts" /> <span>{getT('contacts')}</span>
              </div>
            </li>
          </ul>
        </nav>
        ...
        <div className="sidebar-footer">
          <div className="lang-switcher">
            <span className={language?.toUpperCase() === 'RU' ? 'active-lang' : ''} onClick={() => changeLanguage('RU')}>RU</span> | 
            <span className={language?.toUpperCase() === 'KG' ? 'active-lang' : ''} onClick={() => changeLanguage('KG')}>KG</span> | 
            <span className={language?.toUpperCase() === 'EN' ? 'active-lang' : ''} onClick={() => changeLanguage('EN')}>EN</span>
          </div>
